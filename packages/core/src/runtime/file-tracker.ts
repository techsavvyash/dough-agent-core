...
export class FileTracker {
  ...
  /** Attach (or replace) the persistence backend after construction. */
  setPersistence(persistence: FileTrackerPersistence): void {
...
/**
 * Map file extension → OpenTUI tree-sitter filetype name.
 *
 * OpenTUI bundles parsers for: javascript, typescript, markdown, zig.
 * Aliases recognised by the parser worker:
 *   "typescriptreact" → typescript grammar (handles TSX)
 *   "javascriptreact" → javascript grammar (handles JSX)
 *
 * Other extensions are stored and passed through; the tree-sitter client
 * will silently skip highlighting when no grammar is available.
 */
function extToLanguage(filePath: string): string | undefined {
  const ext = filePath.split(".").pop()?.toLowerCase();
  const map: Record<string, string> = {
    // TypeScript — canonical + react alias (TSX)
    ts:  "typescript",
    tsx: "typescriptreact",
    mts: "typescript",
    cts: "typescript",
    // JavaScript — canonical + react alias (JSX)
    js:  "javascript",
    jsx: "javascriptreact",
    mjs: "javascript",
    cjs: "javascript",
    ...
    zig: "zig",
    zon: "zig",
    // Others (no bundled grammar — stored for forward-compat)
    py:     "python",
    rs:     "rust",
    go:     "go",
    rb:     "ruby",
